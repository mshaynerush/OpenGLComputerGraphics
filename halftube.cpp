...
namespace static_meshes_3D {
	...
	HalfTube::HalfTube(float radius, int numSlices, float height, bool withPositions, bool withTextureCoordinates, bool withNormals)
	...
	float HalfTube::getRadius() const
	{
		return _radius;
	}

	int HalfTube::getSlices() const
	{
		return _numSlices;
	}

	float HalfTube::getHeight() const
	{
		return _height;
	}

	void HalfTube::initializeData()
	{
		if (_isInitialized) {
			return;
		}
		...
		_numVerticesTotal = _numVerticesSide + _numVerticesTopBottom;
	...
	void HalfTube::render() const
	{
		...
		glDrawArrays(GL_TRIANGLE_STRIP, 0, _numVerticesSide / 2);

		// Render top cover
		glDrawArrays(GL_TRIANGLE_FAN, _numVerticesSide, _numVerticesTopBottom / 2 );

		// Render bottom cover
		//glDrawArrays(GL_TRIANGLE_FAN, _numVerticesSide + _numVerticesTopBottom, _numVerticesTopBottom / 2);
	}

	void HalfTube::renderPoints() const
	{
		if (!_isInitialized) {
			return;
		}
